        ...
        m_ce(nullptr)
{
    ...
    // qRegisterMetaType<T>() 是一个用于注册自定义类型的函数。这个函数使得自定义类型能够在Qt的信号和槽机制中使用
    // 参数 "CorrespondencesPtr &" 和 "CorrespondencesPtr" 是 Qt 用来标识该类型的字符串名称。
    // 这里注册了两个类型，一个是 "CorrespondencesPtr &"，一个是 "CorrespondencesPtr"。
...
void Registration::addCorrespondenceRejector()
{
    ...
    // 将用户选择的配准拒绝器对象添加到注册过程中的配准拒绝器列表中
